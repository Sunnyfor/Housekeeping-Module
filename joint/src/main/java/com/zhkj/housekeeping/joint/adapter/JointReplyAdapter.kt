...
class JointReplyAdapter(list: ArrayList<JointBean.Reply>) :
    ...
    override fun onBindViewHolder(holder: BaseRecycleViewHolder, position: Int) {
        ...
        if (getItemViewType(position) == 0) {
            holder.itemView.tv_username_right?.text = ("${timeList?.get(1)} ${getData(position).userName ?: ""}")
            ...
            holder.itemView.tv_username_left?.text = ("${getData(position).userName ?: ""} ${timeList?.get(1)}")
